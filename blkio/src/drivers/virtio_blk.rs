...
impl crate::Queue for Queue<'_> {
    ...
    fn try_enqueue(
        ...
        let result = match req.args {
            ...
            RequestTypeArgs::ReportZones { .. }
            | RequestTypeArgs::Close { .. }
            | RequestTypeArgs::Finish { .. }
            | RequestTypeArgs::Open { .. }
            | RequestTypeArgs::Reset { .. }
            | RequestTypeArgs::CloseAll
            | RequestTypeArgs::FinishAll
            | RequestTypeArgs::OpenAll
            | RequestTypeArgs::ResetAll => {
                completion_backlog.push(Completion::for_failed_req(
                    &req,
                    Errno::NOTSUP,
                    cstr!("request not supported for this driver"),
                ));
                return Ok(());
            }
        };
...
impl Driver for VirtioBlk {
    ...
    fn start(&mut self) -> Result<DriverStartOutcome> {
        ...
        if total_num_queues > max_queues {
            return Err(Error::new(
                Errno::INVAL,
                format!("num_queues + num_poll_queues must not be greater than {max_queues}"),
            ));
        }
